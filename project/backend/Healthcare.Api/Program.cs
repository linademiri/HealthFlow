...
//         new MySqlServerVersion(new Version(8, 0, 33)) // vendos versionin e MySQL që ke
//     ));
...
//         options.UseMySql(
//             configuration.GetConnectionString("DefaultConnection"),
//             new MySqlServerVersion(new Version(8, 0, 32)), // versioni i MySQL tënd
...
void ConfigureServices(IServiceCollection services, IConfiguration configuration)
{
    ...
    services.AddAuthorization(options =>
    {
        options.AddPolicy("RequireDoctorRole", policy => policy.RequireRole("Doctor"));
        options.AddPolicy("RequireAdministratorRole", policy => policy.RequireRole("Administrator"));
        options.AddPolicy("RequireDoctorOrAdministrator", policy => policy.RequireRole("Doctor", "Administrator"));
    });
    ...
    services.AddDbContext<ApplicationDbContext>(options =>
        options.UseMySql(
            configuration.GetConnectionString("DefaultConnection"),
            new MySqlServerVersion(new Version(8, 0, 33)), // vendos versionin tënd të MySQL
            b => b.MigrationsAssembly(typeof(ApplicationDbContext).Assembly.FullName)
        )
    );
